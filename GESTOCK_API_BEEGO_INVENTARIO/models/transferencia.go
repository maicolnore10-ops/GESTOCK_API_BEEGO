...
type Transferencia struct {
	Id                int        `orm:"column(id_transferencia);pk;auto"`
...
func GetTransferenciaById(id int) (v *Transferencia, err error) {
	...
	if err = o.Read(v); err == nil {
		// APLICADO: Carga la relación 'IdProducto' desde la BD para búsquedas individuales
...
func GetAllTransferencia(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Transferencia))
	
	// APLICADO: Fuerza al Query Builder a hacer el JOIN automático en consultas globales
	qs = qs.RelatedSel()
