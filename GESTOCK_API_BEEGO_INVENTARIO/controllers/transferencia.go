...
type TransferenciaController struct {
	beego.Controller
}

// URLMapping ...
func (c *TransferenciaController) URLMapping() {
...
// @Param   body        body    models.Transferencia    true        "body for Transferencia content"
// @Success 201 {int} models.Transferencia
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddTransferencia(&v); err == nil {
			...
			c.Data["json"] = map[string]interface{}{
				"success": false,
				"status":  400,
				"Message": "Error al insertar la transferencia: " + err.Error(),
			}
		}
	} else {
		c.Data["json"] = map[string]interface{}{
...
// @Success 200 {object} models.Transferencia
// @Failure 403 :id is empty
// @router /:id [get]
func (c *TransferenciaController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetTransferenciaById(id)
...
// @Failure 403
// @router / [get]
func (c *TransferenciaController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	l, err := models.GetAllTransferencia(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = map[string]interface{}{
			"success": false,
			"status":  400,
			"Message": "Error al obtener la lista de transferencias: " + err.Error(),
		}
	} else {
		c.Data["json"] = map[string]interface{}{
			"Message": "Usuarios encontrados", // Estándar global unificado para tus colecciones de pruebas en Postman
			"data":    l,
		}
	}
	c.ServeJSON()
...
// @router /:id [put]
func (c *TransferenciaController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
		...
		if err := models.UpdateTransferenciaById(&v); err == nil {
			...
			if errors.Is(err, orm.ErrNoRows) {
				...
				c.Data["json"] = map[string]interface{}{
					"success": false,
					"status":  400,
					"Message": "Error al actualizar la transferencia: " + err.Error(),
				}
			}
		}
	} else {
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *TransferenciaController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	
	if err := models.DeleteTransferencia(id); err == nil {
		c.Data["json"] = map[string]interface{}{
			"success": true,
			"status":  200,
